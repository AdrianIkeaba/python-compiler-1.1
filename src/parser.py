...
class Parser:
    ...
    def for_expr(self):
        ...
        if self.current_tok.type != TT_LPAREN:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected opening '(' parenthesis"
            ))
        ...
        if self.current_tok.type != TT_COMMA:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected comma-separated ',' value"
            ))
        ...
        if self.current_tok.type != TT_RPAREN:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected closing '(' parenthesis"
            ))
    ...
    def statement(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'return', 'continue', 'break', identifier, int, float, '+', '-', '(', 'not', 'if', 'for', 'while' or 'def"
            ))
    ...
    def expr(self):
        ...
        # Peek at the next token to see if this is an assignment
        if self.current_tok.type == TT_IDENTIFIER and self.peek_next().type == TT_EQ:
            var_name = self.current_tok
            res.register_advancement()
        ...
        # Otherwise, parse it as a binary operation or other expression
        node = res.register(self.bin_op(self.comp_expr, ((TT_KEYWORD, 'and'), (TT_KEYWORD, 'or'))))
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected identifier, int, float, '+', '-', '(', 'not', 'if', 'for', 'while' or 'def"
            ))
